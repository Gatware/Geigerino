...
volatile unsigned int DA=0; // Accumula gli impulsi nell'interrupt 0 (sonda A) (impulsi in 1 secondo).
volatile unsigned int DB=0; // Accumula gli impulsi nell'interrupt 1 (sonda B) (impulsi in 1 secondo).
...
String tipo[]={"SBM-19", "SBM-20", "2x SBM-20", "SBM-21", " SI-1G", " SI-3BG", " SI-22G", " SI-29BG", "SBT-9", "SBT-11", "LND-712", "Variabile"};
int    cost[]={   21,      166,       332,         667,      167,        2,        583,       100,      92,       318,      168};
byte   ownb[]={  110,       32,        64,          12,       25,       12,         70,        15,      10,        60,       20}; // Own Background in cpm.
...
void ContaB() // INTERRUPT 1
{
detachInterrupt(1);  
DB++;
if(TS) Tic(); // TIC-TIC SOFTWARE
//while(digitalRead(5)==0); // Usando "FALLING" (fronte di discesa) non dovrebbe servire.
attachInterrupt(1,ContaB,FALLING);
}
