...
pinMode(2,INPUT_PULLUP); // Ingresso sonde Ae B (INT 0).
pinMode(3,OUTPUT); // Altoparlante per Tic-tic.
pinMode(4,INPUT_PULLUP); // Selezione sonda (a GND: Sonda B=esterna).
pinMode(5,INPUT_PULLUP); // Encoder - Pulsante.
...
lcd.print(F("A: ")); lcd.print(tipo[EEPROM.read(1)]);
lcd.setCursor(0,1);
lcd.print(F("B: ")); lcd.print(tipo[EEPROM.read(6)]);
delay(1700);
lcd.clear();
