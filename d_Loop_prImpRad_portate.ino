...
if(millis()-t3>999) // Una volta al secondo:
  ...
  if(!Disp2)
    ...
    {
    dstd=19.6*sqrt(cpm*60/(tempo-(tempo<Ti))); // Adotto coefficiente 1,96 (*10 perché sono decimi) per considerare una confidenza del 95%.
    ...
    if(cpm!=0)
      {             // +5: Aggiunge 0,5 alla deviazione standard (dstd è moltiplicata per 10) per arrotondare.
      dstdPerc=(10*dstd+5)/cpm; lcd.setCursor(10,1); // Es.: 9,6: dstd=96; dstd+5=101, cioè diventa 10,1->10.
      ...
      tempoRes=40000/cpm/Er^2; // Tempo residuo in minuti.
      }
