...
void Azzera()
{Biiip(); cp=0; tempo=0; m=1; m1=1; t3=0; DA=0; DB=0; D=0; DMAX=0; for(m1=1; m1<61; m1++){C[m1]=0;}}
 // END Azzera()
...
lcd.print("    Massimi     ");  
while(digitalRead(5)==LOW) // Attende che venga lasciato il pulsante.
{if(millis()-t1>1000) {Bip(); lcd.clear(); meter(); return;}}  // Premuto per almeno 1 secondo (3,2 totali) salta a massimi(), poi ritorna ed esce.
t1=millis();
...
while(digitalRead(5)==HIGH) // Continua a leggere l'encoder finché non premo
  ...
  lcd.setCursor(6,1); if(LCD==1) lcd.print("On  "); else lcd.print("Auto");
  if(millis()-t1>4999) return; // Dopo 5 secondi di inattività esce.
  }
