...
if(Po==0 && P==0) // Se era ed è premuto per almeno 1,2 secondi
  ...
  if(millis()-t1>1200) {Po=1; Bip(); Integrazione(); while(digitalRead(5)==LOW); Mask(); return;}
  }
...
if(millis()-t3>999) // Una volta al secondo:
  ...
  if(Ti<70)
    {
    C[m]=D; if(D>DMAX){DMAX=D;} D=0; // DMAX: cps massimi (1 secondo).
    if(m<Ti) {m+=1;}
    else {m=1;}
    cp=0;
    for(m1=1; m1<tempo+1; m1++) {cp+=C[m1];} // Somma gli impulsi memorizzati nel tempo Ti.
    }
  else {cp+=D; if(D>DMAX){DMAX=D;} D=0;} // Ti=70, quindi tempo di integrazione infinito)
