...
*   *********  Copyright Gianluca Giangreco 2016-2018  ************   *
*  https://github.com/Gatware/Geigerino/tree/GEIGERINO_1.9cUX_190218 *
*                                                                     *
...
#define SI  988
#define DO 1046
#define TMAX 70 // 60 secondi MAX; TMAX=Continuo; TMX+10=Precisione fissa.
...
// unsigned long XVref=0.940*1072; // 0,94 * Vref in mV. Inserire qui il valore letto con il multimetro sul pin 21 dell'ATmega328P
                       // o il valore del riferimento di tensione esterno montato. (Spostato in setup dopo le letture dell'EEPROM).
...
byte Azz=0; // Contatore sì/no per l'encoder in Azzerare.
...
// unsigned long t2=0; // Lettura del tempo per il Bip - NON PIU' USATO -
...
volatile unsigned int DAB=0; // Accumula gli impulsi nell'interrupt 0 (impulsi in 1 secondo).
unsigned long D=0; // Copia DAB.
unsigned long C[61]; // Impulsi (Count) per ogni secondo (90 secondi MAX; non uso C[0]).
...
int biptic=0; // 0:Nessun suono; 1:Bip; 2:Tic; 3:Bip+Tic.
byte TicSi=0; // Viene impostata 1 volta al secondo per far fare o no il Tic nell'interrupt.
...
byte Disp=0; // 0:uSv/; 1:+/-cpm; 2:Dose
byte Dispo=2; // Valore precedente di Disp.
// byte Disp2=0; // 1:Visualizza la deviazione standard (+/- cpm) al posto dei uSv/h.
...
String tipo[]={"", "SBM-19", "SBM-20", "2xSBM-20", "SBM-21", "SBT-9", "SBT-11A", " SI-1G", " SI-3BG", " SI-22G", " SI-29BG",  "LND-712", "Variabile"};
int    cost[]={0,     21,      166,       332,        667,       92,     318,      167,        2,        583,       100,        168};
byte   ownb[]={0,    110,       16,        22,         12,       10,      12,       25,       12,         70,        15,         20}; // Own Background in cpm.
